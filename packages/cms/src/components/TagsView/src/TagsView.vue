...
const addTags = () => {
  ...
  // console.log(currentRoute.value, 11, menuStore.allMenuRouter)

  if (name) {
    const data = unref(currentRoute)
    const result = menuStore.allMenuRouter.find((item) => item.path === fullPath)

    if (result?.meta.title) data.meta.title = result?.meta.title
    setSelectTag(data)
    tagsViewStore.addView(data)
  }
}
