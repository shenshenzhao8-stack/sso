...
const { tableRegister, tableState, tableMethods } = useTable({
  fetchDataApi: async () => {
    ...
    const res = await getBusinessTagList({
              ...
              <BaseButton
                ...
                onClick={() => {
                  ElMessageBox.confirm('删除不影响历史数据，确定删除该标签？', '提示', {
                    ...
                    deleteBusinessTag(row.id).then((res: any) => {
                      if (res.code === 200) {
...
const save = async () => {
  ...
  if (formData) {
    ...
    if (actionType.value == 'edit') {
      promise = updateBusinessTag(formData as UpdateTagDto)
    } else {
      promise = createBusinessTag(formData as AddTagDto)
    }
    promise
      .then((res) => {
        if (res.code === 200) {
          ElMessage.success(dialogTitle.value + '业务标签成功')
          dialogVisible.value = false
          getList()
        } else if (res.code == 200001 || res.code == 50001 || res.code == 200201) {
          ElMessage.error('业务标签已存在，请重新修改后提交')
        } else {
          ElMessage.error(res.msg)
        }
      })
