...
<template>
  <div class="lock-page fixed inset-0 flex h-screen w-screen bg-black items-center justify-center">
    ...
    <div class="flex w-screen h-screen justify-center items-center">
      <div :class="`lock-page__hour`" class="relative mr-5 md:mr-20 w-2/5 h-2/5 md:h-4/5">
        <span>{{ hour }}</span>
        <span class="meridiem absolute left-5 top-5 text-md xl:text-xl" v-show="showDate">
          {{ meridiem }}
        </span>
      </div>
      <div :class="`lock-page__minute w-2/5 h-2/5 md:h-4/5 `">
        <span> {{ minute }}</span>
      </div>
    </div>
    <transition name="fade-slide">
      <div :class="`lock-page-entry`" v-show="!showDate">
        <div :class="`lock-page-entry-content`">
          ...
          <div :class="`lock-page-entry__footer enter-x`">
            <BaseButton
              type="primary"
              size="small"
