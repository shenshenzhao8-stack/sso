                ...
                {{
                  suffix: () =>
                    showCountDown.value ? (
                      ...
                      <ElButton text onClick={getVerificationCode}>
                        发送
                      </ElButton>
                    )
                }}
  ...
  formRef?.validate(async (valid) => {
    if (valid) {
      ...
      if (res.code === 200) {
        ...
        appStore.setShowResetPassword(false)
...
<template>
  <Dialog v-model="dialogVisible" width="550px" max-height="480px">
    <template #title>
      <div class="m-auto text-16px">首次登录用户，建议修改初始密码，增加账户信息安全</div>
    </template>
    <Form
      :schema="schema"
      :rules="rules"
    ...
    <template #footer>
      <div class="w-[100%] flex">
        <BaseButton type="primary" class="w-[100%]" :loading="loading" @click="resetPassword">
          确认
        </BaseButton>
        <BaseButton type="default" class="w-[100%]" @click="nextResetPassword">
          下次修改
        </BaseButton>
      </div>
    </template>
