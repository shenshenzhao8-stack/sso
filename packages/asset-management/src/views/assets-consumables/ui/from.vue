...
import { ElMessage, ElImage, type UploadRequestOptions, ElButton, ElLoading } from 'element-plus'
...
const { required, lengthRange, _decimalPlaces, _integer } = useValidator()
...
const selectConfigurationDictionary = ref<any>([])

const configurationDictionaryList = async () => {
  const res = await dataConfigurationDictionaryList({ category: 2 })
...
watch(
  ...
  async (currentRow) => {
    ...
    const arr = await configurationDictionaryList()
...
const handleClick = async () => {
  ...
  if (valid) {
    ...
    delete formData.inventory_record_list

    return {
      ...formData,
      img_id: img_id.value,
      price: Number(formData.price),
      stock_num: Number(formData.stock_num)
    }
  }
}
