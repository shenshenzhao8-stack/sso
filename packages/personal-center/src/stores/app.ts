...
export const useAppStore = defineStore(`${import.meta.env.VITE_STORE_PREFIX}_app`, {
  state: (): AppState => {
    return {
      isSubApp: false, // 是否是子应用
      ...
      showResetPassword: false, //是否展示重置密码弹窗
  ...
  actions: {
    setIsSubApp(_isSubApp: boolean) {
      this.isSubApp = _isSubApp
    },
    ...
    setCssVarTheme() {
      for (const key in this.theme) {
        setCssVar(`--${humpToUnderline(key)}`, this.theme[key as keyof typeof this.theme])
      }
      this.setPrimaryLight()
    },
    ...
    setHeaderTheme(color: string) {
      ...
      setCssVar('--top-header-text-color', 'red')
