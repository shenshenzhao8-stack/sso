...
import Write from '@/views/DictionaryManagement/L2Department/components/Write.vue'
...
const submitAction = async () => {
  ...
  if (formData) {
    ...
    if (dialogType.value === 'add') {
      const res = await createDictApi({ name: formData.name, type: 2 })
      submitLoading.value = false
      if (res.code === 200) {
        showAddOrEditDialog.value = false
        ElMessage.success('新增L2部门成功')
        getList()
      } else if (res.code == 100001) {
        ElMessage.error('L2部门名称已存在')
      } else {
